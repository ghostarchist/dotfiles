...
;; create a valid kernel module packge based on the specified kernel.
...
;; This means this means we easily can conditionally add services to the list:
;;
;; @example
;; (list
;;   (simple-service ...)
;;   (simple-service ...)
;;   (when add-keybinding? (simple-service ...)))
;; @end example
...
(define-public (alist->environment-variable var alist)
  ...
  ;; Join arguments into single string, with each key prefixed
  ;; with "--" and the key and value seperated with a space.
  ...
  ;; and its value as the result of seralizing ALIST.
  `((,var . ,str)))
...
;; Covert a font into a pango of fcft string that can be used in
;; configuration files.
